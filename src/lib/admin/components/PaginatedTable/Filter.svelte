...
<div class="row">
    ...
    <div class="col-sm-10">
        {#if filter.type === FilterType.text}
            <input id="input_filter_{filter.name}" class="form-control" type="text" on:change={onChange} bind:value>
        {:else if filter.type === FilterType.date}
            <DatePicker
                ...
                format="YYYY/MM/DD"
                on:date-selected={(e) => {value1 = getDateFromEvent(e); updateValueFromValues()}}
                on:change={onChange}
            />
            <DatePicker
                start={dayjs().subtract(50, 'year')}
