<script lang="ts">
	...
	async function clearFilters() {
		...
		config_filters.forEach((filter: ConfigFilter) => {
			if (filter.element) {
				...
				throw new Error(
					`ConfigFilter with name "${filter.name}" does not have a connected instance`
				);
			}
		});
	}
	...
	async function selectFilter(event) {
		...
		config_filters.forEach((config_filter: ConfigFilter) => {
			...
			let value_index = filters_with_values.findIndex(
				(fv: FilterWithValue) => fv.name === config_filter.name
			);
...
<div id="filters" class="collapse">
	...
	<div class="d-flex" id="filters_actions_container">
		<button
			...
			disabled={disable_save_filters}>💾 Save filters</button
		>
		<input
			class="form-control"
			type="text"
			...
			class:is-invalid={is_filter_name_invalid}
			bind:value={current_filter_name}
		/>
	</div>
</div>
